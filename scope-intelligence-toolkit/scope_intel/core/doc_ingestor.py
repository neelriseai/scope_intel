...
GENERATED_ROUTES: list[tuple] = [
    ...
    # Business/product planning sections that otherwise used to be skipped.
    (r"\bproduct.market fit\b|\bpmf\b|\bpain vs preference\b|\bproof plan\b|\bvalidation plan\b|\bbusiness model\b|\brevenue model\b|\bunit economics\b|\bpricing\b", None, "roadmap"),
...
def _update_claude_md(repo_root: Path, project_name: str, generated: list[dict]) -> None:
    """Upsert scope-intel context in root CLAUDE.md and .claude/CLAUDE.md."""
    marker = "<!-- scope-intel-doc-context -->"
    end_marker = "<!-- end scope-intel-doc-context -->"
    doc_lines = [
        f"\n{marker}",
    ...
    def _upsert(path: Path, header: str) -> None:
        if path.exists():
...
def _ingest_with_llm(
    ...
    *,
    ...
    for i, chunk in enumerate(chunks, 1):
        ...
        classification_from_llm = classification is not None
        target_file = None
        ...
        if chunk["title"] == "(preamble)" and chunk.get("text", "").strip():
            ...
            classification.setdefault("section_title", "Project Overview")
            classification.setdefault("key_facts", [])
            classification.setdefault("constraints", [])
            classification.setdefault("is_feature", False)
            classification.setdefault("feature_id", "")
            classification.setdefault("summary", chunk.get("text", "")[:200])
            classification.setdefault("tags", [])

        if target_file:
            if classification_from_llm:
                llm_count += 1
            else:
                fallback_count += 1
        elif classification:
    ...
    if second_pass:
        ...
        file_summaries = [
            {
                "title": data["title"],
                "summary": " ".join(data["summaries"][:3]) or " ".join(
                    (s.get("text") or s.get("body_text", ""))[:300]
                    for s in data.get("sections", [])[:2]
                ),
            }
            for tf, data in buckets.items()
            if TARGET_FILE_MAP.get(tf, ("",))[0] == "generated"
        ]
        if file_summaries:
            module_map_content = llm.module_map_pass(file_summaries)
            if module_map_content:
                existing_sections = buckets.get("module-map.md", {}).get("sections", [])
                sections = [{
                ...
                if existing_sections:
                    source_text = "\n\n".join(
                        (s.get("text") or s.get("body_text", "")).strip()
                        for s in existing_sections
                        if (s.get("text") or s.get("body_text", "")).strip()
                    )
                    if source_text:
                        sections.append({
                            "heading_path": "Source Sections Classified as Module Map",
                            "title": "Source Sections Classified as Module Map",
                            "text": source_text,
                        })
    ...
    routed_count = sum(1 for e in routing_table if e.get("file"))
    ...
    return {
        ...
        "routing_coverage":      {
            ...
            "sections_routed": routed_count,
            "sections_unmatched": len(set(unmatched)),
            "coverage_percent": round((routed_count / len(chunks)) * 100, 2) if chunks else 0.0,
        },
...
def _ingest_python_only(
    ...
    *,
    ...
    for s in sections:
        # Text before the first heading is usually the executive summary. Route
        # it to overview before keyword matching can steal it for architecture.
    ...
    result: dict = {
        ...
        "routing_coverage":   {
            ...
            "sections_routed": routed_count,
            "sections_unmatched": len(unmatched),
            "coverage_percent": round((routed_count / len(sections)) * 100, 2) if sections else 0.0,
        },
        "files_written":      written_count,
